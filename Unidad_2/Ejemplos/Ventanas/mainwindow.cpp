...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), configRGB({0,0,0}) // necesario inicializar para evitar error por parte de usuario
...
void MainWindow::on_btnCambiarFondo_clicked()
{
    // Para cambiar solo fondo
    QColor colorFondo(configRGB[0], configRGB[1], configRGB[2]);
    QPalette paletaColor = ui->centralwidget->palette();
    paletaColor.setColor(QPalette::Window, colorFondo);

    ui->centralwidget->setPalette(paletaColor);
    ui->centralwidget->setAutoFillBackground(true);

    qDebug()<< "[Mainwindow] Cambiar fondo RGB ";
}
