...
**$f(x) = x^3 - 5x^2 + 7x - 2.2$**, donde la variable $x$ representa la apertura angular del disco de la válvula medida en grados. 
Dentro del rango de operación segura de **0.5 a 1.8** grados, determine el punto de turbulencia exacto deteniendo el cálculo 
cuando el error porcentual respecto a la **raíz real de 1.8252111** grado sea menor o igual al **0.01%**.
...
la función matemática: **$f(x) = 0.125x^2-0.7x+0.7$**, donde la variable $x$ representa el volumen de 
aire insuflado medido en litros. Dentro del rango de volumen seguro de **1.0 a 3.0** litros, determine el punto 
exacto de equilibrio de presión ejecutando el cálculo durante exactamente 8 iteraciones frente al valor real de **1.3 litros**.

> **Respuesta**: biseccion

## Problema 4
La cantidad de oxígeno que absorbe un tejido artificial en un laboratorio se modela mediante la 
función matemática: **$f(x) = 0.02x^2-0.6x+1$**, donde la variable x representa el 
...
real de **1.7712** semanas sea menor o igual al **0.05%**.

> **Respuesta**: Newton-Raphson - bisección

## Problema 5
El flujo de líquido nutritivo a través de un canal de cultivo celular se modela mediante 
la función matemática: **$f(x) = 0.5x^3 - 4x + 2$**, donde la variable x representa la velocidad de 
bombeo medida en mililitros por minuto. Partiendo de una estimación inicial de **1.0 mililitros** 
por minuto. Calcule el punto exacto de equilibrio del flujo frente al valor **real de 0.517304** mililitros por minuto.
...
voltios. Determine tensión exacta de activación, deteniendo el cálculo de forma inmediata en cuanto el error 
porcentual respecto al **valor real de 2.19148** voltios sea menor o igual al **0.01%**.
