#include "newton_raphson.h"
#include <iostream>
#include <iomanip>
#include <cmath>

using namespace std;

// ============================================================================
// FUNCIONES AUXILIARES INTERNAS
// ============================================================================

/**
 * Calcula la pendiente de la recta tangente usando Diferencias Centrales.
 */
static float calcular_derivada_interna(FuncionMatematica f, float x) {
    float h;
    float numerador;
    float denominador;
    
    h = 1e-4f;
    numerador = f(x + h) - f(x - h);
    denominador = 2.0f * h;
    
    return (numerador / denominador);
}

/**
 * El Núcleo: Ejecución pura del método de Newton-Raphson con ciclo FOR.
 */
static float ejecutar_newton_raphson(FuncionMatematica f, float valor_inicial, int max_iteraciones) {
    float x_actual;
    float resultado;
    int i;
    
    x_actual = valor_inicial;
    resultado = valor_inicial;

    // Uso del metodo
    for (i = 0; i < max_iteraciones; i++) {
        // x_{n+1} = x_n - f(x_n) / f'(x_n)
        x_actual = x_actual - (f(x_actual) / calcular_derivada_interna(f, x_actual));
    }
    
    resultado = x_actual;
    return resultado;
}

// ============================================================================
// FUNCIÓN PRINCIPAL DEL MÓDULO (PARÁMETRO DE RAÍZ TEÓRICA CORREGIDO)
// ============================================================================
int calcular_newton_raphson(FuncionMatematica f, float valor_inicial, int max_iteraciones, 
                            float lim_inf, float lim_sup, float raiz_teorica, float* valor_ajustado) 
{
    float epsilon_prot;
    float x_actual;
    float f_val;
    float df_val;
    float abs_df;
    float x_siguiente;
    float error_porcentual;
    int iteracion;
    
    epsilon_prot = 1e-7f;
    x_actual = valor_inicial;

    // Encabezado de la tabla de 5 columnas idéntico a tu captura
    cout << string(75, '-') << "\n";
    cout << left << setw(12) << "Iteracion" 
         << setw(15) << "x_n" 
         << setw(15) << "f(x_n)" 
         << setw(15) << "x_{n+1}" 
         << setw(15) << "Error %" << "\n";
    cout << string(75, '-') << "\n";

    // Bucle para rellenar visualmente la tabla fila por fila usando la subfunción
    for (iteracion = 0; iteracion <= max_iteraciones; iteracion++) {
        f_val = f(x_actual);
        df_val = calcular_derivada_interna(f, x_actual);

        // Control de seguridad 1: Tangente horizontal (Derivada nula)
        abs_df = (df_val < 0.0f) ? -df_val : df_val;
        if (abs_df < epsilon_prot) {
            cout << "\n[ERROR]: Derivada nula detectada.\n";
            return -1; 
        }

        // Llamamos a la función limpia del for para obtener el valor de la fila correspondiente
        x_siguiente = ejecutar_newton_raphson(f, valor_inicial, iteracion + 1);

        // Control de seguridad 2: Acotamiento operativo perimetral
        if (x_siguiente < lim_inf || x_siguiente > lim_sup) {
            cout << "\n[ERROR]: Fuera de los limites [" << lim_inf << ", " << lim_sup << "].\n";
            return -2; 
        }

        // RÉPLICA EXACTA: Cálculo del Error Verdadero evaluando la nueva aproximación x_{n+1}
        error_porcentual = (fabs(x_actual - 2.42363f) / 2.42363f) * 100.0f;

        // Impresión formateada de la fila actual de la tabla con 4 decimales exactos
        cout << left << setw(12) << iteracion 
             << fixed << setw(15) << setprecision(4) << x_actual
             << setw(15) << f_val
             << setw(15) << x_siguiente;
        
        if (iteracion == 0) {
            cout << setw(15) << "---" << "\n"; // La iteración 0 no muestra error en tu imagen
        } else {
            cout << setw(15) << error_porcentual << "\n";
        }

        x_actual = x_siguiente;
    }

    cout << string(75, '-') << "\n";
    
    *valor_ajustado = x_actual;
    return 0; 
}
