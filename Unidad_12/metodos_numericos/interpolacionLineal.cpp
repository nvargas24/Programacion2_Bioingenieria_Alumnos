...
int8_t generar_tabla_estimaciones(const PuntoInterpolacion_t* tabla_nodos, uint16_t tamano_nodos,
    ...
    for (uint16_t i = 0; i < max_pasos; i++) {
        // 1. Asignar NMuestra y calcular la entrada de manera equidistante
        buffer_salida[i].n_muestra = i + 1;
        buffer_salida[i].valor_entrada = limite_inferior + ((float)i * delta_paso);

        // 2. Ejecutar el método de interpolación y consultar la curva real
        float x_actual = buffer_salida[i].valor_entrada;
        ...
        // 3. Cálculo del error absoluto para procesar el porcentaje
        float error_abs = fabsf(estimado - real);

        // 4. Lógica de blindaje del divisor analizada y corregida en GeoGebra
        if (real == 0.0f || estimado == 0.0f) {
